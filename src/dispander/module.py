...
async def dispand(message):
    ...
    for m in messages:
        ...
        #     icon_url=getattr(main_embed.author, "icon.url", EmptyEmbed),
...
def compose_embed(message):
    ...
    #         embed.set_author(name=message.author.name,icon_url="https://media.discordapp.net/avatars/{}/{}.png?size=1024".format(message.author.id, message.author.avatar.key),url=message.jump_url)
    # else:
    #     embed.set_author(name=message.author.name,url=message.jump_url)
    
    if hasattr(message.author.avatar, 'key'): #アイコン画像が設定されているとき
            embed.set_author(name=message.author.display_name,icon_url="https://media.discordapp.net/avatars/{}/{}.png?size=1024".format(message.author.id, message.author.avatar.key),url=message.jump_url)
    else:
